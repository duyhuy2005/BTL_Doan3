...
// Key lưu người dùng dùng chung với trang Quản Lý Người Dùng
var USERS_STORAGE_KEY = 'shopvn_users';

function loadUsersForAuth() {
  try {
...
function saveUsersForAuth(list) {
  try {
...
function getInitialsForAuth(name) {
  if (!name) return '??';
...
function initSignup() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Lưu khách hàng vào shopvn_users để dùng chung với admin-users
    let users = loadUsersForAuth();

    // Không cho trùng email
    if (users.some(u => u.email === email)) {
      showAlert('Email đã tồn tại trong hệ thống, vui lòng dùng email khác hoặc đăng nhập.', 'error');
      return;
    }
    ...
    const newUser = {
      ...
      status: 'active',
      createdAt: today,
      avatar: getInitialsForAuth(name)
    };

    users.push(newUser);
    saveUsersForAuth(users);
